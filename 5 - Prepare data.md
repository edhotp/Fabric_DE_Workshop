...
In this tutorial, you use notebooks with [Spark runtime](https://learn.microsoft.com/en-us/fabric/data-engineering/runtime) to transform and prepare raw data in your lakehouse.

## Prerequisites

Before you begin, you must complete the previous tutorials in this series:

1. [Create a lakehouse](https://learn.microsoft.com/en-us/fabric/data-engineering/tutorial-build-lakehouse)
2. [Ingest data into the lakehouse](https://learn.microsoft.com/en-us/fabric/data-engineering/tutorial-lakehouse-data-ingestion)
3. Ensure [lakehouse schemas](https://learn.microsoft.com/en-us/fabric/data-engineering/lakehouse-schemas) are enabled in your lakehouse.
...
2. In your browser, go to your Fabric workspace in the [Fabric portal](https://app.fabric.microsoft.com/).
3. Select **Import** > **Notebook** > **From this computer**.

    [![Screenshot showing the import notebook option in the Fabric portal.](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/import-notebook.png)](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/import-notebook.png#lightbox)

4. Select **Upload** from the **Import status** pane that opens on the right side of the screen.
5. Select only the notebook that matches your preferred coding language.
...
7. After the import is successful, go to the items view of the workspace to verify the imported notebook.

    [![Screenshot showing the list of imported notebooks and where to select the lakehouse.](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/imported-notebooks-lakehouse.png)](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/imported-notebooks-lakehouse.png#lightbox)

8. Select the **wwilakehouse** lakehouse to open it, so that the notebook you open next is linked to it.
9. From the top navigation menu, select **Open notebook** > **Existing notebook**.

    [![Screenshot showing the list of successfully imported notebooks.](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/existing-notebook-ribbon.png)](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/existing-notebook-ribbon.png#lightbox)

10. Select your imported notebook for PySpark or Spark SQL and select **Open**. The notebook is already linked to your opened lakehouse, as shown in the lakehouse **Explorer**.
...
1. **Cell 1 - Spark session configuration.** This cell enables two Fabric features that optimize how data is written and read in subsequent cells. [V-order](https://learn.microsoft.com/en-us/fabric/data-engineering/delta-optimization-and-v-order) optimizes the parquet file layout for faster reads and better compression. [Optimize write](https://learn.microsoft.com/en-us/fabric/data-engineering/delta-optimization-and-v-order#what-is-optimize-write) reduces the number of files written and increases individual file size.
...
2. **Cell 2 - Fact - Sale.** This cell reads raw parquet data from `Files/wwi-raw-data/full/fact_sale_1y_full`, adds date part columns (**Year**, **Quarter**, and **Month**), and writes `fact_sale` as a Delta table partitioned by **Year** and **Quarter**.
...
3. **Cell 3 - Dimensions.** This cell reads the five dimension parquet datasets and writes them as Delta tables (`dimension_city`, `dimension_customer`, `dimension_date`, `dimension_employee`, and `dimension_stock_item`) under `Tables/dbo/...`.
...
4. To validate the created tables, right-click the **wwilakehouse** lakehouse in the explorer and then select **Refresh**. The tables appear.

    [![Screenshot showing where to find your created tables in the Lakehouse explorer.](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/tutorial-lakehouse-explorer-tables.png)](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/tutorial-lakehouse-explorer-tables.png#lightbox)
...
2. **Cell 4 - Load source tables for transformation (PySpark only).** If you're using the PySpark notebook, run this cell to load Delta tables into DataFrames for the aggregation steps that follow.
...
3. **Cell 5 - Create `aggregate_sale_by_date_city`.** This cell joins sales, date, and city data, then creates the city-level aggregate table.
...
4. **Cell 6 - Create `aggregate_sale_by_date_employee`.** This cell joins sales, date, and employee data, then creates the employee-level aggregate table.
...
5. To validate the created tables, right-click the **wwilakehouse** lakehouse in the explorer and then select **Refresh**. The aggregate tables appear.

    [![Screenshot of the Lakehouse explorer showing where the new tables appear.](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/validate-tables.png)](https://learn.microsoft.com/en-us/fabric/data-engineering/media/tutorial-lakehouse-data-preparation/validate-tables.png#lightbox)
...
> [Create a semantic model and build a report](https://learn.microsoft.com/en-us/fabric/data-engineering/tutorial-lakehouse-build-report)
